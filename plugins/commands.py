...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('✪ 𝖲𝗎𝗉𝗉𝗈𝗋𝗍 𝖦𝗋𝗈𝗎𝗉', url="https://t.me/Discussion_tr_links"),
                    InlineKeyboardButton('⌬ 𝖬𝗈𝗏𝗂𝖾𝗌 𝖦𝗋𝗈𝗎𝗉', url="https://t.me/Tr_Movies_Request_Factory")
                ],[
                    InlineKeyboardButton('✇ 𝖩𝗈𝗂𝗇 𝖴𝗉𝖽𝖺𝗍𝖾𝗌 𝖢𝗁𝖺𝗇𝗇𝖾𝗅 ✇', url='https://t.me/Tr_LinksZz')
                  ]]
    ...
    if len(message.command) != 2:
        buttons = [[
                    ...
                    InlineKeyboardButton('★ 𝖢𝗁𝖺𝗇𝗇𝖾𝗅', url='https://t.me/Tr_LinksZz'),
                    InlineKeyboardButton('♧︎︎︎ 𝖬𝗈𝗏𝗂𝖾 𝖦𝗋𝗈𝗎𝗉', url="https://t.me/Discussion_tr_links")
                ],[
    ...
    if not files_:
        ...
        try:
            ...
            title = '@Tr_LinksZz  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@Tr_LinksZz  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@Tr_LinksZz  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]
